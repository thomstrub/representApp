...
class Division(BaseModel):
    ...
    """
    ocd_id: str = Field(..., description="OCD division identifier", pattern=r"^ocd-division/country:us/.*")
    ...
    has_data: bool = Field(..., description="Whether OpenStates returned representatives for this division")
...
class Representative(BaseModel):
    ...
    office: str = Field(..., max_length=100, description="Position title (e.g., 'US Senator', 'State Senator - District 12')")
    party: Optional[str] = Field(None, description="Party affiliation (Democratic, Republican, Independent, etc.)")
    email: Optional[str] = Field(None, description="Email address or semicolon-separated emails")
    phone: Optional[str] = Field(None, pattern=r"^\d{3}-\d{3}-\d{4}$", description="Phone number in XXX-XXX-XXXX format")
    address: Optional[str] = Field(None, max_length=500, description="Office mailing address")
    website: Optional[str] = Field(None, description="Official website URL")
    photo_url: Optional[str] = Field(None, description="Photo URL (passed through without validation)")
    government_level: str = Field(
        ..., 
...
class Metadata(BaseModel):
    ...
    government_levels: List[str] = Field(
        ...
        description="Which government levels are included in response"
    )
    response_time_ms: Optional[int] = Field(None, description="End-to-end processing time in milliseconds")
...
class AddressLookupResponse(BaseModel):
    ...
    representatives: List[Representative] = Field(
        default_factory=list,
        description="Array of representatives (deduplicated by id)"
    )
