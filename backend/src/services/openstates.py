...
class OpenStatesClient:
    ...
    def get_representatives_by_division(self, ocd_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if govt_level != "state":
                logger.info(
                    f"Skipping non-state division for OpenStates: {ocd_id} (level: {govt_level})"
                )
        ...
        params = {"jurisdiction": jurisdiction, "per_page": 50}  # OpenStates API max is 50
    ...
    @tracer.capture_method
    def get_representatives_by_coordinates(
        self, latitude: float, longitude: float
    ) -> List[Dict[str, Any]]:
        """
        Get representatives by geographic coordinates using OpenStates /people.geo endpoint
        ...
        Notes:
            - Empty results (no representatives at coordinates) return empty list, not exception
            - Uses 10-second timeout for API calls
        """
        # T028: Validate coordinates
        ...
        logger.info(f"Looking up representatives by coordinates: lat={latitude}, lng={longitude}")
        tracer.put_annotation(key="latitude", value=str(latitude))
        tracer.put_annotation(key="longitude", value=str(longitude))

        # T029: Make API request to geo endpoint
        endpoint = f"{self.base_url}/people.geo"
        headers = {"X-API-Key": self.api_key, "Accept": "application/json"}
        ...
        try:
            ...
            # T030: Handle 429 - Rate limit exceeded
            if response.status_code == 429:
                logger.error("Rate limit exceeded for OpenStates API")
                error_data = response.json() if response.text else {}
                error_message = error_data.get("error", "Rate limit exceeded")
            ...
            # Handle empty results (no data for coordinates)
            if not results:
                logger.info(
                    f"No representatives found for coordinates: lat={latitude}, lng={longitude}"
                )
                tracer.put_annotation(key="representative_count", value=0)
                return []

            # T031-T032: Transform OpenStates response to Representative model format
            representatives = []
            ...
            for person_data in results:
                ...
                # Transform using helper method
                representative = self._transform_openstates_person_to_representative(person_data)
                representatives.append(representative)

            logger.info(
                f"Successfully retrieved {len(representatives)} representatives by coordinates"
            )
            tracer.put_annotation(key="representative_count", value=len(representatives))
        ...
        # Handle network timeouts
            ...
            logger.error(f"Timeout calling OpenStates geo API: {str(e)}")
            raise ApiException(
                code=ErrorCode.EXTERNAL_SERVICE_ERROR,
                message="OpenStates API request timed out. Service may be unavailable.",
                status_code=503,
        ...
        # Handle connection errors
            ...
            logger.error(f"Connection error calling OpenStates geo API: {str(e)}")
            raise ApiException(
                code=ErrorCode.EXTERNAL_SERVICE_ERROR,
                message="Unable to connect to OpenStates API. Service may be unavailable.",
                status_code=503,
            ...
            # Catch any other unexpected errors
            logger.error(f"Unexpected error calling OpenStates geo API: {str(e)}")
            raise ApiException(
                code=ErrorCode.EXTERNAL_SERVICE_ERROR,
                message="An unexpected error occurred while calling OpenStates geo API",
                status_code=503,
                details=str(e),
            )
    ...
    def _transform_openstates_person_to_representative(
        ...
        """
        Transform OpenStates person data to Representative model format

        Args:
            person_data: Raw person data from OpenStates API

        Returns:
            Dictionary with Representative model fields
        """
        # Extract current role information
        current_role = person_data.get("current_role", {})
    ...
    def _parse_government_level_from_division_id(self, division_id: str) -> str:
        """
        Parse government level from OCD division ID
        ...
        # Federal patterns
        if "/cd:" in division_id:  # Congressional district
            return "federal"

        # State patterns
        if "/sldl:" in division_id or "/sldu:" in division_id:  # State legislative
            return "state"

        # Local patterns
        if "/place:" in division_id or "/county:" in division_id:
            return "local"

        # Default to state if pattern not recognized
        return "state"
