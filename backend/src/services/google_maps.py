...
from ..utils.parameter_store import get_google_maps_api_key
...
class GoogleMapsClient:
    ...
    def __init__(self):
        """
        Initialize Google Maps client with API key from Parameter Store
        
        Raises:
            ApiException: If API key cannot be retrieved
        """
        try:
            ...
            # Re-raise parameter store exceptions as-is
            raise
        except Exception as e:
            logger.error(f"Failed to initialize Google Maps client: {str(e)}")
            raise ApiException(
                code=ErrorCode.INTERNAL_ERROR,
                message="Failed to initialize Google Maps service",
                status_code=500,
                details=str(e)
            )
    ...
    def geocode(self, address: str, timeout: int = 5) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            logger.info(f"Geocoding address with timeout {timeout}s", extra={"address_length": len(address)})
