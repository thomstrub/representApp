...
class TestGoogleMapsClient:
    ...
    def setup_method(self):
        ...
        # Mock googlemaps client during setup
        with patch("src.services.google_maps.googlemaps.Client") as mock_client_class:
            ...
            # Pass API key directly as parameter (matches OpenStatesClient pattern)
            self.client = GoogleMapsClient(api_key="test-api-key")
    ...
    def test_authentication_error_raises_api_exception(self):
        ...
        assert exc_info.value.status_code == 503
        assert (
            "authentication" in exc_info.value.message.lower()
            or "api key" in exc_info.value.message.lower()
        )
        assert "Google Maps" in exc_info.value.message
